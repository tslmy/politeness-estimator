...
def mask(content):
    ...
    masked_content = whitespace_pattern.sub(
        " {SP} ", masked_content
    )  # Reserve natural whitespaces
...
assert (
    mask(
        "@李名扬: 哈喽❤~你来看看{LK}3JKS2L 这个里面有没有 @郭德纲 说的那个#宝藏#^_^。我觉得  还可以！"
    )
    == "哈喽 ❤ ~你来看看 {LK}  {SP} 这个里面有没有 {SP}  {AT}  {SP} 说的那个 {TP} ^_^。我觉得 {SP} 还可以！"
)
